...
from models.model_3 import Decoder
import argparse
import time
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, boundary=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    for sal in out[0]:
        sal_loss += bce_iou(sal, label, alpha, beta)
    for edge in out[1]:
        edge_loss += bce_iou(edge, boundary, alpha, beta / 30.)
    fuse_loss = bce_iou(out[2], label, alpha, beta)
