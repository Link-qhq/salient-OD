...
from models.model_4 import Decoder
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, boundary=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    extra_loss = (bce_iou(out[2], label, alpha, beta) +
                  bce_iou(out[3], label, alpha, beta) +
                  bce_iou(out[4], label, alpha, beta)) / 3.0
    return sal_loss + edge_loss + extra_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=list, default=[0.5, 0.5, 1.0], help="loss weight")
