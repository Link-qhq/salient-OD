...
# F.interpolate(feat, **kwargs, mode="bilinear", align_corners=False)
...
class CSC(nn.Module):
    ...
    def forward(self, left, right, mode='last'):
        if mode != 'last':
...
class CSFI(nn.Module):
    ...
    def forward(self, x):
        ...
        # first conv
        x_h = self.relu(self.bnh_0(self.h2h_0(x)))  # 右
        x_l = self.relu(self.bnl_0(self.h2l_0(self.h2l_pool(x))))  # 左

        # mid conv
        x_h2h = self.contrast3(x_h)  # 经过contrast层 右
        x_h2h = self.h2h_1(x_h2h)
        x_l2l = self.contrast1(x_l)  # 经过contrast层 左
