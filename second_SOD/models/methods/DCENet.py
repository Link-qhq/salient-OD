...
class DCENet(nn.Module):
    def __init__(self, backbone_path=None):
        ...
        # backbone
        resnet50 = ResNet()
        self.layer0 = nn.Sequential(resnet50.conv1, resnet50.bn1, nn.ReLU(inplace=True))
        self.layer1 = nn.Sequential(nn.MaxPool2d(3, 2, 1), resnet50.layer1)
...
if __name__ == '__main__':
    ...
    edge = torch.rand(1, 1, 56, 56).to('cuda:0')
    model = DCENet().to('cuda:0')
    ouput = model(input)
    # for i in range(8):
    #     print(ouput[i].shape)
