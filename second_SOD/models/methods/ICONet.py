...
class DFA(nn.Module):
    ...
    def __init__(self, x, y, dilation=2):
        ...
        self.atrConv = nn.Sequential(
            nn.Conv2d(x, y, kernel_size=3, dilation=dilation, padding=dilation, stride=1), nn.BatchNorm2d(y), nn.PReLU()
        )
...
class ICE(nn.Module):
    ...
    def forward(self, in1, in2=None, in3=None):
        if in2 is not None and in1.size()[2:] != in2.size()[2:]:
            # in2 = F.interpolate(in1, size=in1.size()[2:],
            #                     mode='bilinear')  # Here, I made a bug at first. Sorry about it.
            in2 = F.interpolate(in2, size=in1.size()[2:],
                                mode='bilinear')  # If you want to re-train it, you can comment out last line and use this.
        else:
            in2 = in1
        ...
        context = (x.pow(2).sum((2, 3), keepdim=True) + self.eps).pow(0.5)  # [B, C, 1, 1]  L2正则化
...
class ICONet(nn.Module):
    def __init__(self, model_name='ICON-R'):
        ...
        set_channel = 64
        self.dfa1 = DFA(2048, set_channel)
        self.dfa2 = DFA(1024, set_channel)
        self.dfa3 = DFA(512, set_channel)
        self.dfa4 = DFA(256, set_channel)

        self.ice1 = ICE()
        self.ice2 = ICE()
        self.pwv = PWV(set_channel)
        self.fuse = nn.Sequential(nn.Conv2d(set_channel, set_channel, kernel_size=1),
                                  nn.Conv2d(set_channel, set_channel, kernel_size=3, padding=1),
                                  nn.BatchNorm2d(set_channel), nn.ReLU(inplace=True))

        self.predtrans1 = nn.Conv2d(set_channel, 1, kernel_size=3, padding=1)
        self.predtrans2 = nn.Conv2d(set_channel, 1, kernel_size=3, padding=1)
        self.predtrans3 = nn.Conv2d(set_channel, 1, kernel_size=3, padding=1)
        self.predtrans4 = nn.Conv2d(set_channel, 1, kernel_size=3, padding=1)
        self.predtrans5 = nn.Conv2d(set_channel, 1, kernel_size=3, padding=1)
