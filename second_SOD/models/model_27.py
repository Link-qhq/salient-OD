...
from models.modify import MBIG
...
from models.utils import cus_sample
...
class BCM(nn.Module):
    def __init__(self, in_ch):
        ...
        self.conv1 = conv3x3_bn_pRelu(in_ch, in_ch)
        self.conv2 = conv3x3_bn_pRelu(in_ch, in_ch)
        self.conv3 = conv3x3_bn_pRelu(in_ch, in_ch)
        self.conv4 = conv3x3_bn_pRelu(in_ch, in_ch)
...
class Decoder(nn.Module):
    def __init__(self, img_size=224):
        ...
        self.big = MBIG(set_channels)
        self.bcm = BCM(set_channels)
        self.conv_out = nn.Conv2d(set_channels, 1, 1, 1)
        self.conv_bcm = nn.Conv2d(set_channels, 1, 1, 1)

    def forward(self, fea, shape):
        # fea1, fea2, fea3, fea4 = self.msfam(fea[0], fea[1], fea[2], fea[3])
        ...
        out = self.conv_out(self.big(out1_3, bcm))
        ...
        # bcm = self.conv_bcm(F.interpolate(bcm, size=shape, mode='bilinear', align_corners=True))  # 边缘预测
        # out = self.big(out1_3, bcm)
        # out = self.conv_out(F.interpolate(out, size=shape, mode='bilinear', align_corners=True))
