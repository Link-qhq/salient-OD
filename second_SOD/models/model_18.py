...
from timm.models import SENet
from timm.models.senet import SEResNetBottleneck
...
class HFSM(nn.Module):
    ...
    def __init__(self, in_channel, depth):
        ...
        self.conv = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 1, 1), nn.BatchNorm2d(self.depth),
                                  nn.ReLU(inplace=True))

        self.senet_layer6 = SENet(SEResNetBottleneck, [4, 4, 4, 4], 1, 16, in_chans=self.depth)
        self.senet_layer12 = SENet(SEResNetBottleneck, [4, 4, 4, 4], 1, 16, in_chans=self.depth)
        self.senet_layer18 = SENet(SEResNetBottleneck, [4, 4, 4, 4], 1, 16, in_chans=self.depth)

        self.atrous_block1 = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 1, 1), nn.BatchNorm2d(self.depth),
                                           nn.ReLU(inplace=True))
        self.atrous_block6 = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 3, 1, padding=2, dilation=2),
                                           nn.BatchNorm2d(self.depth), nn.ReLU(inplace=True))
        self.atrous_block12 = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 3, 1, padding=3, dilation=3),
                                            nn.BatchNorm2d(self.depth), nn.ReLU(inplace=True))
        self.atrous_block18 = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 3, 1, padding=4, dilation=4),
                                            nn.BatchNorm2d(self.depth), nn.ReLU(inplace=True))

        self.conv12 = nn.Sequential(nn.Conv2d(self.depth * 2, self.depth, 1, 1), nn.BatchNorm2d(self.depth),
                                    nn.ReLU(inplace=True))
        self.conv18 = nn.Sequential(nn.Conv2d(self.depth * 3, self.depth, 1, 1), nn.BatchNorm2d(self.depth),
                                    nn.ReLU(inplace=True))

        self.conv_output = nn.Sequential(nn.Conv2d(self.depth * 5, self.depth, 1, 1), nn.BatchNorm2d(self.depth),
                                         nn.ReLU(inplace=True))

        self.conv3 = nn.Sequential(nn.Conv2d(self.in_channel, self.depth, 1, 1, 0), nn.BatchNorm2d(self.depth),
                                   nn.ReLU(inplace=True))
...
class feature_exctraction(nn.Module):
    def __init__(self, in_channel, depth, kernel):
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(self.depth, self.depth, self.kernel, 1, (self.kernel - 1) // 2),
                                   nn.BatchNorm2d(self.depth), nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv2d(self.depth, self.depth, 3, 1, 1), nn.BatchNorm2d(self.depth),
                                   nn.ReLU(inplace=True))
...
class SANet(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.dim = in_dim
        self.k = 9
        self.conv1_1 = nn.Sequential(nn.Conv2d(in_dim, self.dim // 2, (1, self.k), 1, (0, self.k // 2)),
                                     nn.BatchNorm2d(self.dim // 2), nn.ReLU(inplace=True))
        self.conv1_2 = nn.Sequential(nn.Conv2d(in_dim, self.dim // 2, (self.k, 1), 1, (self.k // 2, 0)),
                                     nn.BatchNorm2d(self.dim // 2), nn.ReLU(inplace=True))
...
class PDRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return predict1, predict2, predict3, predict4, predict5, predict6, predict1_fore, predict1_back, predict2_fore, predict2_back, predict1_edge, predict2_edge, (
                                                                                                                                                                                 1 - out3_attention)[
                                                                                                                                                                     :,
                                                                                                                                                                     :1,
                                                                                                                                                                     :,
