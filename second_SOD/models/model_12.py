...
class LocalAttention(nn.Module):
    def __init__(self, dim=64):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, groups=hidden_dim), nn.BatchNorm2d(hidden_dim), nn.ReLU(True)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, groups=hidden_dim), nn.BatchNorm2d(hidden_dim),
            nn.ReLU(True)
        )
        ...
        self.pre_conv = conv1x1_bn_relu(dim, dim)
        self.conv = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, groups=dim), nn.BatchNorm2d(dim), nn.ReLU(True)
        )
    ...
    def forward(self, x, H, W):
        ...
        x = x.view(b, c, H, W)
        # x = self.pre_conv(x)
        x1, x2, x3, x4 = torch.split(x, 16, 1)
        x1 = self.conv1(x1)
        x2 = self.conv2(x2)
        x3 = self.conv3(x3)
        x4 = self.conv4(x4)
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):
        norm = self.norm1(x)
        x = x + self.drop_path(self.attn1(norm, H, W) + self.attn2(norm, H, W))
