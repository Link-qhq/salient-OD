...
class LocalAttention(nn.Module):
    def __init__(self, channel=64, kernel_size=7):
        ...
        pad = kernel_size // 2
        hidden_channel = channel // 2
        # self.conv_h1 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=7, padding=3), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        # self.conv_h2 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=7, padding=3), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        ...
        #
        # self.conv_w1 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=7, padding=3), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        # self.conv_w2 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=7, padding=3), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
    ...
    def forward(self, x, H, W):
        ...
        # h1, h2 = torch.split(x_h, 32, dim=1)
        # w1, w2 = torch.split(x_w, 32, dim=1)
        # x_h = torch.cat([self.conv_h1(h1), self.conv_w2(w2)], 1)
        # x_w = torch.cat([self.conv_w1(w1), self.conv_h2(h2)], 1)
...
class LocalATT(nn.Module):
    def __init__(self, channel=64):
        ...
        hidden = channel // 4
        self.conv = conv1x1_bn_relu(channel, channel)
        self.conv1 = nn.Sequential(
            nn.Conv2d(hidden, hidden, kernel_size=(1, 3), stride=1, padding=(0, 1), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            nn.Conv2d(hidden, hidden, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            # conv1x1_bn_relu(hidden, hidden),
            conv3x3_bn_relu(hidden, hidden, stride=1, padding=1, dilate=1)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(hidden, hidden, kernel_size=(1, 3), stride=1, padding=(0, 1), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            nn.Conv2d(hidden, hidden, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            # conv1x1_bn_relu(hidden, hidden),
            conv3x3_bn_relu(hidden, hidden, stride=1, padding=2, dilate=2)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(hidden, hidden, kernel_size=(1, 3), stride=1, padding=(0, 1), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            nn.Conv2d(hidden, hidden, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            # conv1x1_bn_relu(hidden, hidden),
            conv3x3_bn_relu(hidden, hidden, stride=1, padding=3, dilate=3)
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(hidden, hidden, kernel_size=(1, 3), stride=1, padding=(0, 1), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            nn.Conv2d(hidden, hidden, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=hidden), nn.BatchNorm2d(hidden), nn.ReLU(True),
            # conv1x1_bn_relu(hidden, hidden),
            conv3x3_bn_relu(hidden, hidden, stride=1, padding=5, dilate=5)
        )
        self.fuse_conv = conv1x1_bn_relu(channel, channel)

    def forward(self, x, h, w):
        b, _, c = x.shape
        x = x.view(b, c, h, w)
        x1, x2, x3, x4 = torch.split(x, 16, dim=1)
        x1 = self.conv1(x1)
        x2 = self.conv2(x2)
        x3 = self.conv3(x3)
        x4 = self.conv4(x4)
        out = self.fuse_conv(torch.cat([x1, x1 + x2, x1 + x2 + x3, x1 + x2 + x3 + x4], 1))
        out = F.sigmoid(out)
        return out.view(b, _, c)
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):
        # x = x + self.drop_path(self.attn1(self.norm1(x), H, W))
