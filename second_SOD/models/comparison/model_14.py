...
from models.methods.MINet import SIM
...
class Decoder(nn.Module):
    def __init__(self, in_channel_list, img_size=224, channel=64):
        ...
        # spatial enhancement
        self.pe_att1 = nn.Sequential(nn.Conv2d(set_channels, 1, 7, 1, 3), self.up, nn.Sigmoid())
        self.pe_att2 = nn.Sequential(nn.Conv2d(set_channels, 1, 7, 1, 3), self.up, nn.Sigmoid())
        self.pe_att3 = nn.Sequential(nn.Conv2d(set_channels, 1, 7, 1, 3), self.up, nn.Sigmoid())
        self.pe_att4 = nn.Sequential(nn.Conv2d(set_channels, 1, 7, 1, 3), self.up, nn.Sigmoid())
        # depths = [1, 1, 1, 1]
        # num_stages = 4
...
if __name__ == '__main__':
    ...
    model = Net(backbone='resnet', decoder=Decoder).to('cuda:0')
    ouput = model(input)
    # for i in range(8):
    #     print(ouput[i].shape)
