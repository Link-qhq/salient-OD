...
class MFIM(nn.Module):
    def __init__(self, channel):
        ...
        self.atr5_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(self.channel_single), nn.ReLU(True),
            conv1x1_bn_relu(self.channel_single, self.channel_single),
            nn.Conv2d(self.channel_single, self.channel_single, kernel_size=7, stride=1, padding=3, dilation=1),
            nn.BatchNorm2d(self.channel_single), nn.ReLU(True)
        )
        self.pool_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(self.channel_single), nn.ReLU(True),
            conv1x1_bn_relu(self.channel_single, self.channel_single),
            nn.Conv2d(self.channel_single, self.channel_single, kernel_size=9, stride=1, padding=4, dilation=1),
            nn.BatchNorm2d(self.channel_single), nn.ReLU(True)
        )
...
if __name__ == '__main__':
    ...
    model = Net(backbone='res', decoder=Decoder).to('cuda:0')
    ouput = model(input)
    # for i in range(8):
    #     print(ouput[i].shape)
