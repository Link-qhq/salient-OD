...
from models.module.common import conv3x3_bn_relu, conv1x1_bn_relu, weight_init
import torch.nn.functional as F
from models.module.attention import SpatialAttention, AttentionBlock, \
    SpaceAttention, CBAM, SCSA_SA, ChannelAttention, SEAttention
from models.backbone.PVT_V2 import Attention, Mlp
from einops import rearrange
from models.methods.ICONet import ICE
...
class LocalAttention(nn.Module):
    def __init__(self, channel=64, kernel_size=7):
        ...
        self.conv_h1 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=3, padding=1), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        self.conv_h2 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=3, padding=1), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        ...
        self.conv_w1 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=3, padding=1), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
        self.conv_w2 = nn.Sequential(nn.Conv1d(hidden_channel, hidden_channel, kernel_size=3, padding=1), nn.BatchNorm1d(hidden_channel), nn.ReLU(True))
    ...
    def forward(self, x, H, W):
        ...
        x_h = torch.cat([self.conv_h1(h1), self.conv_h2(h2)], 1)
        w1, w2 = torch.split(x_w, 32, dim=1)
        x_w = torch.cat([self.conv_w1(w1), self.conv_w2(w2)], 1)
...
class ATN(nn.Module):
    ...
    def forward(self, x, H, W):
        b, _, c = x.shape
        x = x.view(b, c, H, W)
        return (self.ca(x) * self.sa(x)).view(b, _, c)
...
class Decoder(nn.Module):
    def __init__(self, in_channel_list, img_size=224, channel=64):
        ...
        self.se_att1 = ICE(set_channels)
        self.se_att2 = ICE(set_channels)
        self.se_att3 = ICE(set_channels)
        self.se_att4 = ICE(set_channels)
...
if __name__ == '__main__':
    ...
    input = torch.rand(2, 3, 320, 320).to('cuda:0')
    edge = torch.rand(2, 1, 56, 56)
    model = Net(backbone='pvt', decoder=Decoder).to('cuda:0')
    ouput = model(input)
    # for i in range(8):
    #     print(ouput[i].shape)
