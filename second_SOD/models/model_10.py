...
class MFIM(nn.Module):
    ...
    def forward(self, x):
        ...
        I2 = self.fuse_atr2(torch.cat([I1, x], 1))
        I2 = self.atr2_conv(I2)
        # I2 = self.fuse_atr2(torch.cat([I1, I2], 1))

        I3 = self.fuse_atr3(torch.cat([I2, x], 1))
        I3 = self.atr3_conv(I3)
        # I3 = self.fuse_atr3(torch.cat([I2, I3], 1))

        I4 = self.fuse_atr4(torch.cat([I3, x], 1))
        I4 = self.atr5_conv(I4)

        I5 = self.fuse_atr5(torch.cat([I4, x], 1))
        I5 = self.pool_conv(I5)
