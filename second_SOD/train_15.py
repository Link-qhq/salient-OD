...
from models.model_15 import PGN
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    for i in range(len(loss_weights)):
        loss_list.append(structure_loss(out[i], label) * loss_weights[i])
...
def fit(model, train_dl, args, file_fold):
    ...
    for epoch in range(args.last_epoch, epochs):
        ...
        for i, data_batch in enumerate(progress_bar):
            iters = iters + 1
            images, label = data_batch[0].to(args.device, non_blocking=True).float(), data_batch[1].to(args.device, non_blocking=True).float()
            if args.amp:  # 混合精度训练
...
def test(args):
    ...
    model = PGN()
    model.to(args.device)
    model.load_state_dict(torch.load(save_path + '150_200' + '.pth', weights_only=True))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='Model_15', type=str, help='M3Net with different backbone')
    ...
    parser.add_argument('--save_epoch', default=[120, 150, 160, 180], type=list, help='salient map index')
    ...
    parser.add_argument('--beta', type=float, default=0.5, help="loss coefficient")
