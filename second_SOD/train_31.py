...
from models.model_29 import PGN
...
def compute_loss(out, label, boundary=None, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    all_loss = bce_iou(out[0], label, alpha, beta) + theta * wbce(out[1], boundary, window_size=3)
    return all_loss
...
def test(args):
    ...
    model = PGN()
    model.load_state_dict(torch.load(save_path + args.method + '.pth'))
    # model.load_state_dict(torch.load(save_path + '80_100.pth', weights_only=True))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='Model_31', type=str, help='M3Net with different backbone')
    ...
    parser.add_argument('--alpha', type=float, default=3.0, help="loss coefficient")
    parser.add_argument('--beta', type=float, default=1.0, help="loss coefficient")
    parser.add_argument('--theta', type=float, default=3.0, help="loss coefficient")
    parser.add_argument('--weights', type=list, default=[1.0], help="loss weight")
