...
from models.model_21 import PGN
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    for i in range(len(loss_weights)):
        loss_list.append(structure_loss(out[i], label, alpha, beta) * loss_weights[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='Model_21', type=str, help='M3Net with different backbone')
    ...
    parser.add_argument('--weights', type=list, default=[0.5, 0.5, 1.0, 1.0, 1.0, 1.0], help="loss weight")
