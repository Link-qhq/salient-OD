...
from models.model_28 import PGN
...
def compute_loss(out, label, boundary=None, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    # all_loss = loss_fn(out[0], label, alpha, beta) + loss_fn(out[1], boundary, alpha, beta / 30.)
    all_loss = 0.
    for i, x in enumerate(out):
        all_loss = all_loss + bce_iou(x, label, alpha, beta) * loss_weights[i]
    return all_loss
    # for i in range(len(loss_weights)):
    #     loss_list.append(bce_wiou(out[i], label, alpha, beta) * loss_weights[i])
...
def fit(model, train_dl, args, file_fold):
    ...
    # valid
    # best_MAE = 1000.0
    # test_dataset = TestDataSet('datasets/', ['DUTS-TE'], 352, 'test')
    # test_loader = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False, num_workers=12)
    for epoch in range(args.last_epoch, epochs):
        ...
        for i, data_batch in enumerate(progress_bar):
            iters = iters + 1
            images, label = data_batch[0].to(args.device, non_blocking=True).float(), \
        ...
        # # valid
        # mae = MAE()
        # progress_bar = tqdm(test_loader, desc='DUTS-TE', ncols=140)
        # with torch.no_grad():
        #     for i, data_batch in enumerate(progress_bar):
        #         images = data_batch[0]
        #         label = data_batch[1]
        #         images = images.to(args.device).float()
        #         outputs_saliency = model(images, shape=data_batch[2])[args.salient_idx]
        #         pred = torch.sigmoid(outputs_saliency)[0, 0].cpu().numpy()
        #         mae.step(pred, label.cpu().numpy())
        #         progress_bar.set_postfix(mae="{:.5f}".format(mae.get_results()['mae']))
        # current = mae.get_results()['mae']
        # if current < best_MAE:
        #     best_MAE = current
        #     torch.save(model.state_dict(), file_fold + 'best.pth')  # gpu Tensor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='Model_28', type=str, help='M3Net with different backbone')
