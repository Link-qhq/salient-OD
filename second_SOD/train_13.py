...
from models.model_13 import EDN
...
def fit(model, train_dl, args, file_fold):
    ...
    def get_lr(warm_epoch, num_epoch, lr):
        def lr_fun(cur_epoch):
            # return (1 - abs((cur_epoch + 1) / (num_epoch + 1) * 2 - 1)) * lr
    ...
    for epoch in range(args.last_epoch, epochs):
        ...
        opt.zero_grad()
        opt.param_groups[0]['lr'], opt.param_groups[1]['lr'] = update_lr(args, opt, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_epochs', default=32, type=int, help='total training epochs')
    parser.add_argument('--img_size', default=352, type=int, help='network input size')
    parser.add_argument('--method', default='Model_13', type=str, help='M3Net with different backbone')
    ...
    parser.add_argument('--betas', default=(0.9, 0.99), type=tuple, help='beats coefficient')
    ...
    parser.add_argument('--weights', type=list, default=[0.5, 0.5, 0.5, 0.25, 0.125, 0.0625], help="loss weight")
