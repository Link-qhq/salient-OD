...
from models.model_20 import PGN
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    # loss_list = []
    loss = bce_iou(out[0], label, alpha, beta)
    for x in out[1]:
        loss = loss + 0.5 * bce_iou(x, label, alpha, beta)
    for x in out[2]:
        loss = loss + 0.5 * bce_iou(x, label, alpha, beta / 30)
    return loss
    # 总损失
...
def fit(model, train_dl, args, file_fold):
    ...
    for epoch in range(args.last_epoch, epochs):
        ...
        for i, data_batch in enumerate(progress_bar):
            iters = iters + 1
            images, label, edge = data_batch[0].to(args.device, non_blocking=True).float(), \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='Model_20', type=str, help='M3Net with different backbone')
