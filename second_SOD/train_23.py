...
from models.model_23 import PGN
...
def weighted_bce(pred, gt):
    weit = 1 + 4 * torch.abs(F.avg_pool2d(gt, kernel_size=31, stride=1, padding=15) - gt)
    wbce = (F.binary_cross_entropy_with_logits(pred, gt, reduction='none') * weit).sum(dim=[2, 3]) / weit.sum(dim=[2, 3])
    return wbce.mean()


def confident_loss(pred, gt, beta=2, lmbda=0.3):
    y = torch.sigmoid(pred)
    weight = beta * y * (1 - y)
    weight = weight.detach()
    loss = (F.binary_cross_entropy_with_logits(pred, gt, reduction='none') * weight).mean()
    loss2 = lmbda * beta * (y * (1 - y)).mean()
    return loss + loss2
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    for sal_pred, wght in zip(out, weight):
        # print(sal_pred)
        if count == 0:  # Second stage forecast
            stage_sal_loss = weighted_bce(sal_pred, label)
        else:
            stage_sal_loss = confident_loss(sal_pred, label, beta=2)
            if count % 2 == 0:  # Second stage forecast
                stage_sal_loss = weighted_bce(sal_pred, label)
        sal_loss = sal_loss + wght * stage_sal_loss
        count += 1
    return sal_loss
    # 总损失
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_epochs', default=200, type=int, help='total training epochs')
    parser.add_argument('--img_size', default=224, type=int, help='network input size')
    parser.add_argument('--method', default='Model_23', type=str, help='M3Net with different backbone')
    ...
    parser.add_argument('--amp', default=False, type=bool, help="amp train")
    parser.add_argument('--salient_idx', default=7, type=int, help='salient map index')
    parser.add_argument('--save_epoch', default=[50, 55, 60], type=list, help='salient map index')
    ...
    parser.add_argument('--coe', default=1, type=float, help='learning coefficient')
    ...
    # loss function
    parser.add_argument('--loss_fun', type=str, default="bce_dice", help="adopt loss function")
