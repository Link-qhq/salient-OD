...
def main():
    ...
    gt_root = os.path.join('D:/working/salient_object_detection/datasets/datasets/'+dataset, 'gt')
    pred_root = os.path.join('preds/', dataset)
    ...
    for gt_name in tqdm(gt_name_list, total=len(gt_name_list)):
        ...
        if gt.shape != pred.shape:
            cv2.imwrite(os.path.join('util/evaltool/Prediction/' + method + '/' + dataset + '/', gt_name),
                        cv2.resize(pred, gt.shape[::-1]))
        # print('OK')
        pred = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", default='ECSSD')
