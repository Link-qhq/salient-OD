...
import argparse
from test_16 import testing
from compute_metric import start
...
def compute_loss(out, label, loss_function="bce_iou", loss_weights=None, boundary=None, alpha=1.0, beta=1.0, theta=1.0):
    ...
    for i in range(len(loss_weights)):
        loss_list.append(loss_fun(out[i], label, alpha, beta) + loss_fun(out[i + 4], boundary, alpha, beta))
...
def fit(model, train_dl, test_dl, args, file_fold):
    ...
    for epoch in range(args.last_epoch, epochs):
        ...
        if (epoch + 1) % 80 == 0 or (epoch + 1) % 90 == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--warm_up', default=100, type=int, help='warm up epochs')
    ...
    # loss function
    parser.add_argument('--loss_fun', type=str, default="bce_wiou", help="adopt loss function")
    ...
    parser.add_argument('--pre_path', default='results/exp_45/Model_16.pth', type=str, help='pre-trained path')
    parser.add_argument('--last_epoch', default=0, type=int, help='last saved epoch')

    # test
    parser.add_argument('--save_path', default='results/exp_49/', type=str, help='save model path')
    parser.add_argument('--save_test', default='preds/', type=str, help='save saliency maps path')
    parser.add_argument('--test_methods', type=str, default='ECSSD+DUTS-TE+DUT-O+HKU-IS+PASCAL-S+SOD',
