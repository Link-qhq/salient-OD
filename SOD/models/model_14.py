...
from models.module.upgrade import BIG
...
class DFA(nn.Module):
    ...
    def __init__(self, x, y):
        ...
        self.conv1_avgP = nn.Sequential(
            nn.Conv2d(x, y, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(y),
            nn.AvgPool2d(kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(y)
        )
        self.conv1_maxP = nn.Sequential(
            nn.Conv2d(x, y, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(y),
    ...
    def forward(self, f):
        p1 = self.oriConv(f)  # 3x3 原始卷积
        p2 = self.asyConv(f)  # 3x3 非对称卷积
        p3 = self.atrConv1(f)  # 2x2 空洞卷积
        p4 = self.atrConv2(f)  # 3x3 空洞卷积
        p5 = self.conv1_avgP(f)  # 1x1 + avgPool
        p6 = self.conv1_maxP(f)  # 1x1 + maxPool
        p7 = self.conv1_k(f)  # 1x1 + 3x3
        p8 = self.conv1x1(f)  # 1x1
...
class SDU(nn.Module):
    def __init__(self, channel=256, groups=32, ratio=4):
        ...
        self.pr1_2 = nn.Sequential(
            ...
            nn.GroupNorm(groups // ratio, channel // ratio),
            nn.PReLU(),
            nn.Conv2d(channel // ratio, 1, 1)
        )
