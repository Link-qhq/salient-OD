...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fusion = CSC(set_channels)
        self.csf1 = CSFI(set_channels, set_channels // 2)
        self.csf2 = CSFI(set_channels, set_channels // 2)
        self.csf3 = CSFI(set_channels, set_channels // 2)
        self.csf4 = CSFI(set_channels, set_channels // 2)
    ...
    def forward(self, fea, shape):
        ...
        fea_4 = self.fusion(fea_4, sppf, mode='last')

        fea_4_ = self.csf4(fea_4)
        fea_4_ = fea_4_.reshape(B, C, -1).transpose(1, 2)
        fea_4 = fea_4.reshape(B, C, -1).transpose(1, 2)

        fea_3 = self.mib3(fea_3, fea_4)
        out3 = fea_3
        fea_3 = fea_3.transpose(1, 2).reshape(B, C, _h // 4, _w // 4)
        fea_3_ = self.csf3(fea_3)
        fea_3_ = fea_3_.reshape(B, C, -1).transpose(1, 2)

        fea_2 = self.mib2(fea_2, out3, fea_4_)
        fea_1 = self.mib1(fea_1, fea_2, fea_3_)

        fea_1 = fea_1.transpose(1, 2).reshape(B, C, _h, _w)
        out = fea_1
        fea_1 = self.csf1(fea_1)
        fea_2 = fea_2.transpose(1, 2).reshape(B, C, _h // 2, _w // 2)
        fea_2 = self.csf2(fea_2)

        fea_3 = fea_3_.transpose(1, 2).reshape(B, C, _h // 4, _w // 4)
        fea_4 = fea_4_.transpose(1, 2).reshape(B, C, _h // 8, _w // 8)
