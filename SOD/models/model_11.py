...
from models.module.attention import CBAM, SpatialAttention
...
class DFA(nn.Module):
    ...
    def __init__(self, x, y, dilation=2):
        ...
        self.atrConv1 = nn.Sequential(
            nn.Conv2d(x, y, kernel_size=3, dilation=2, padding=2, stride=1),
            nn.BatchNorm2d(y)
        )
        self.atrConv2 = nn.Sequential(
            nn.Conv2d(x, y, kernel_size=3, dilation=3, padding=3, stride=1),
            nn.BatchNorm2d(y)
        )
    ...
    def forward(self, f):
        ...
        p3 = self.atrConv1(f)  # 2x2 空洞卷积
        p4 = self.atrConv2(f)  # 3x3 空洞卷积
        p5 = self.conv1_p(f)  # 1x1 + avgPool
        p6 = self.conv1_k(f)  # 1x1 + 3x3
        p7 = self.conv1x1(f)  # 1x1
...
class BIG(nn.Module):
    ...
    def forward(self, x, edge):
        ...
        # 全局池化
        gv1 = F.avg_pool2d(e1, (e1.size(2), e1.size(3)), stride=(e1.size(2), e1.size(3)))  # [B, C // 4, 1, 1]
...
class ICE(nn.Module):
    def __init__(self, num_channels=64, ratio=8):
        ...
        self.space_add_conv = SpatialAttention()
        self.conv = nn.Sequential(
